...
function getBufferClass(score) {
    if (score >= 80) return "buffer-strong";
    if (score >= 60) return "buffer-supported";
    if (score >= 40) return "buffer-warning";
    return "buffer-danger";
}
...
function computeTaxableSS(ssAnnual, filingStatus) {
    ...
    // Provisional income = SS only (can be expanded later)
...
document
    ...
    .addEventListener("change", e => {
        document.getElementById("customVolInputs").style.display = e.target.checked
            ? "block"
            : "none";
    });

function applyWithdrawals({ age, roth, trad, spendingNeed, ssIncome, retireTax }) {
...
function computeFirstYearWithdrawals(engineYears, retirementAge) {
    ...
    return {
        ...
        rothFirstYearWithdrawal: yr?.rothWithdrawal ?? 0
    };
}
...
function simulateRothConversions({
    ...
    // RMD at 73 using IRS divisor 26.5 (approx)
...
$("runBtn").addEventListener("click", async () => {
    ...
    function buildDeterministicChart({
        ...
        for (let i = 0; i < totalYears; i++) {
            ...
            if (age < retirementAge) {
                rothContribution = contribution * rothPct * (1 - currentTax);
                tradContribution = contribution * tradPct;
    ...
    /* ---------------------------------------------------
       BUILD CURVES & PHASES (used elsewhere)
    --------------------------------------------------- */

    const curves = buildYearlyCurves(
        engineYears,
        result.withdrawalReport?.combinedDepletionAge ?? result.depletionAge
    );
...
function renderTaxChart({ contribution, expectedReturn, years, currentTax, rothFinal }) {
    ...
    taxChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: {
                    title: { display: true, text: "Retirement Tax Rate" }
                },
                y: {
                    ticks: { callback: v => formatCurrency(v) },
                    title: { display: true, text: "After‑Tax Value" }
                }
            }
        }
    });

    // Insert narrative below the chart
...
function generateGuidance(result) {
    ...
    if (!retirementTaxDetails) {
        return [{ type: "neutral", text: "No guidance available for this scenario." }];
    }

    const { rmd, ssAtClaimAge, estimatedRate, otherWithdrawals } = retirementTaxDetails;
...
function computeProInsights(result) {
    ...
    let tradDepletionAge = null;
    let rothDepletionAge = null;
    ...
    function runReadinessMonteCarlo({
        ...
        for (let i = 0; i < simulations; i++) {
            ...
            if (balance >= readinessThreshold) successCount++;
        }
    ...
    function withdrawalInsight(balance, rate, growthRate, years) {
        ...
        return { rate, annual: balance * rate, endBalance, ratio, label };
    }

    const { currentRoth, currentTrad, retirementTaxDetails, taxContext } = result;
    ...
    if (retirementTaxDetails && taxContext) {
        ...
        rmdPressureScore = Math.round(Math.max(0, Math.min(100, raw * 60)));
        ...
        const currentBracket =
            brackets.find(b => taxable <= b.top) || brackets[brackets.length - 1];
        ...
        if (safeConversionMax !== null && safeConversionMax > 0) {
            ...
            const conversionGrowthRate = parseFloat(result.assumedGrowthRate) || 0.07;
        ...
        const userAllowsTradGrowth =
            result.userTradContribution > 0 || result.currentTrad > 0;

        if (!userAllowsTradGrowth) {
            tradBalance = 0;
            tradAtRetirement = 0;
            tradDepletionAge = result.taxContext.retirementAge;
        ...
        fourPercent = withdrawalInsight(retirementBalance, 0.04, growthRate, yearsInRetirement);
        fivePercent = withdrawalInsight(retirementBalance, 0.05, growthRate, yearsInRetirement);
        ...
        // If plan lasts to 120 with huge buffer, it's green
        ...
        if (catastrophic) {
            ...
            !simulationStrong && (
                requiredWithdrawalRate > 0.045 ||
                safeSpendingDelta > 0 ||
                (sustainabilityFailureAge != null && sustainabilityFailureAge < 95) ||
...
function renderWithdrawalStrategy(result, stressAge) {
    ...
    setText("required-withdrawal-rate", formatPercent(data.requiredWithdrawalRate));
    ...
    setText("withdrawal-sequencing-note", data.withdrawalStrategyLabel);
...
function renderPositiveSustainability({
    depletionAge, yearsLeft, withdrawalRate, spendingGap, successRate, bufferScore, result
}) {
...
function renderYellowSustainability({
    depletionAge, yearsLeft, withdrawalRate, spendingGap, bufferScore, result
}) {
    const dAge = safeNumber(depletionAge);
    const yLeft = safeNumber(yearsLeft);
    const wRate = safeNumber(withdrawalRate);
...
function buildChartDepletionDiagnostic({
    ...
    const result = { mismatchType: "none", explanationKey: null };
...
function messageClassicSafe(result) {
    const buffer = Number.isFinite(result.bufferScore) ? result.bufferScore : 0;
...
// Spending message dispatcher
function getSpendingMessage(result) {
    const tier = classifySpendingTier(result);

    switch (tier) {
        case "classic-safe": return messageClassicSafe(result);
        case "elevated-supported": return messageElevatedSupported(result);
        case "aggressive-but-supported": return messageAggressiveSupported(result);
        default: return messageUnsustainable(result);
    }
}
...
function renderSpendingMessage(insights) {
    ...
    const buffer = Number.isFinite(insights.bufferScore) ? insights.bufferScore : 0;
...
// Chart mismatch message dispatcher
function getChartMismatchMessage(result) {
    ...
    switch (diag.explanationKey) {
        case "chart_mismatch_trad_early": return messageChartTradEarly(result);
        case "chart_mismatch_roth_early": return messageChartRothEarly(result);
        case "chart_mismatch_healthy_but_finite": return messageChartHealthyButFinite(result);
        case "chart_mismatch_positive_but_depletes": return messageChartPositiveButDepletes(result);
        default: return null;
    }
}
...
function renderSafeSpending(result) {
    ...
    setText("safe-spending-range", `${formatCurrency(low)} – ${formatCurrency(high)}`);
    setText(
    ...
    if (retirementReadiness !== null) {
        ...
        html += `
            <div class="pro-insights-metric">
                ...
                <div class="readiness-score ${readinessClass}">${retirementReadiness}/100</div>
                <div class="readiness-label ${readinessClass}">${readinessLabel}</div>
...
function renderSummary(data) {
    ...
    setText("combined-depletion-age", combinedAge ? `Age ${combinedAge}` : "N/A");
...
`;
    ...
    renderChartMismatchMessage(getChartMismatchMessage(data));
    ...
    if (slider) {
        slider.addEventListener("input", () => {
            ...
            const { filingStatus, currentTax, retirementAge, rmd } = data.taxContext;
            ...
            if (currentTax < data.taxContext.retireTax && annualConversion > 0) {
